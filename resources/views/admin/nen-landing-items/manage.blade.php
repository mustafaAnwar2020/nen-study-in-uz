                ...
                <div class="card-body">
                    <form action="{{ route('admin.nen-landing-items.store', $resource) }}" method="post" id="form">
                        ...
                        @elseif($resource === 'partners')
                            <x-localized-field :source="$row ?? null" name="name" label="Name" :value="$row->name ?? ''" :value-ar="$row->name_ar ?? ''" required />
                            <x-localized-field :source="$row ?? null" name="description" label="Description" type="textarea" :rows="2" :value="$row->description ?? ''" :value-ar="$row->description_ar ?? ''" />
                        ...
                        @elseif($resource === 'faqs')
                            <x-localized-field :source="$row ?? null" name="question" label="Question" :value="$row->question ?? ''" :value-ar="$row->question_ar ?? ''" required />
                            <x-localized-field :source="$row ?? null" name="answer" label="Answer" type="textarea" :rows="3" :value="$row->answer ?? ''" :value-ar="$row->answer_ar ?? ''" />
                        ...
                        @elseif($resource === 'feature-cards')
                            <div class="row">
                                ...
                                <div class="col-md-9"><x-localized-field :source="$row ?? null" name="stat_label" label="Stat Label" :value="$row->stat_label ?? ''" :value-ar="$row->stat_label_ar ?? ''" /></div>
                            </div>
                            <x-localized-field :source="$row ?? null" name="title" label="Title" :value="$row->title ?? ''" :value-ar="$row->title_ar ?? ''" required />
                            <x-localized-field :source="$row ?? null" name="description" label="Description" type="textarea" :rows="2" :value="$row->description ?? ''" :value-ar="$row->description_ar ?? ''" />
                        ...
                        @elseif($resource === 'how-it-works')
                            <div class="row">
                                ...
                                <div class="col-md-10"><x-localized-field :source="$row ?? null" name="title" label="Title" :value="$row->title ?? ''" :value-ar="$row->title_ar ?? ''" required /></div>
                            </div>
                            <x-localized-field :source="$row ?? null" name="description" label="Description" type="textarea" :rows="2" :value="$row->description ?? ''" :value-ar="$row->description_ar ?? ''" />
                        ...
                        @elseif($resource === 'translation-agencies')
                            <x-localized-field :source="$row ?? null" name="name" label="Agency Name" :value="$row->name ?? ''" :value-ar="$row->name_ar ?? ''" required />
                            <x-localized-field :source="$row ?? null" name="service_description" label="Service Description" :value="$row->service_description ?? ''" :value-ar="$row->service_description_ar ?? ''" />
                            <div class="form-group"><label>Logo / Image</label><x-file-upload class="form-control" data-folder="nen-landing" name="image-file" /></div>
                            <div class="row">
                                <div class="col-md-6"><x-localized-field :source="$row ?? null" name="location" label="City / Location" :value="$row->location ?? ''" :value-ar="$row->location_ar ?? ''" /></div>
                        ...
                        @elseif($resource === 'documents')
                            <x-localized-field :source="$row ?? null" name="title" label="Document Title" :value="$row->title ?? ''" :value-ar="$row->title_ar ?? ''" required />
                            <x-localized-field :source="$row ?? null" name="description" label="Description / Note" type="textarea" :rows="2" :value="$row->description ?? ''" :value-ar="$row->description_ar ?? ''" />
                            <div class="form-group"><label>Icon Image</label><x-file-upload class="form-control" data-folder="nen-landing" name="image-file" /></div>
