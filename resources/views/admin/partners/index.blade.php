                            ...
                            <div class="card-body p-0 table-responsive">
                                <table class="dataTableRows table table-bordered table-hover">
                                    ...
                                    <tr>
                                        ...
                                        <th class="text-center">Webiste</th>
                                    ...
                                    @foreach($rows as $idx=>$row)
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                ...
                                                <button
                                                        @if($row->is_active)
                                                        onclick="destroy('{{route('admin.change_status', ['model'=>'Partner','model_id'=>$row->id, 'status'=>'inactive'])}}', 'Sure to deactivate?')"
                                                        @else
                                                        onclick="destroy('{{route('admin.change_status', ['model'=>'Partner','model_id'=>$row->id, 'status'=>'active'])}}', 'Sure to activate?')"
                                                        @endif
                                                        class="btn btn-{{$row->is_active ? 'warning' : 'success'}} btn-sm">
                                                    <i class="fas fa-power-off"></i>
                                                </button>

                                                <a href="{{route('admin.partners.edit', $row->slug)}}"
                                                   class="btn btn-dark btn-sm">
                                                    <i class="fas fa-pen"></i>
                                                </a>

                                                <button
                                                        onclick="destroy('{{route('admin.delete', ['model'=>'Partner','model_id'=>$row->id])}}', 'Sure to delete this data?')"
