                    ...
                    <div class="col-md-12">
                        <div class="card">
                            ...
                            <div class="card-body p-0 table-responsive">
                                <form action="{{ route('admin.tpi-key-benefits-section.update') }}" method="post" id="form">
                                    @csrf
                                    <div class="modal-body">
                                        <div class="row">
                                            ...
                                            <div class="col-md-12">
                                                <div class="form-group">
                                                    <label>Benefit Cards (icon, icon_class, title, description)</label>
                                                    <div id="tpi-key-benefits-container">
                                                        @php $items = $row->getItemsList(); @endphp
                                                        @forelse($items as $index => $item)
                                                            @php $item = (array) $item; @endphp
                                                            <div class="border rounded p-3 mb-3 tpi-key-benefit-row">
                                                                <div class="row mb-2">
                                                                    <div class="col-md-2">
                                                                        <input type="text" class="form-control" name="items[{{ $index }}][icon]"
                                                                               value="{{ $item['icon'] ?? '' }}" placeholder="e.g. bi-gift-fill">
                                                                    </div>
                                                                    <div class="col-md-2">
                                                                        <input type="text" class="form-control" name="items[{{ $index }}][icon_class]"
                                                                               value="{{ $item['icon_class'] ?? '' }}" placeholder="e.g. icon-success">
                                                                    </div>
                                                                    <div class="col-md-5">
                                                                    ...
                                                                    <div class="col-md-2">
                                                                        <button type="button" class="btn btn-success add-key-benefit {{ $loop->last ? '' : 'd-none' }}">+</button>
                                                                        <button type="button" class="btn btn-danger remove-key-benefit {{ $loop->first ? 'd-none' : '' }}">−</button>
                                                                    </div>
                                                                </div>
                                                                <div class="row">
                                                                    <div class="col-md-11">
                                                        ...
                                                        @empty
                                                            <div class="border rounded p-3 mb-3 tpi-key-benefit-row">
                                                                <div class="row mb-2">
                                                                    ...
                                                                    <div class="col-md-5"><input type="text" class="form-control" name="items[0][title]" placeholder="Title"></div>
                                                                    <div class="col-md-2">
                                                                        <button type="button" class="btn btn-success add-key-benefit">+</button>
                                                                        <button type="button" class="btn btn-danger remove-key-benefit d-none">−</button>
                                                                    </div>
                                                                </div>
                                                                <div class="row">
...
@push('scripts')
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '.add-key-benefit', function () {
                const $row = $(this).closest('.tpi-key-benefit-row');
                ...
                $newRow.find('.add-key-benefit').addClass('d-none');
                $newRow.find('.remove-key-benefit').removeClass('d-none');
                $('#tpi-key-benefits-container').append($newRow);
                rowIndex++;
            });
            $(document).on('click', '.remove-key-benefit', function () {
                $(this).closest('.tpi-key-benefit-row').remove();
            });
        });
    </script>
@endpush
